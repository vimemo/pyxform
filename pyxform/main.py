...
def main():
    ...
    # TODO: don't call surveys "SomeName" by default
    sname = "SomeName"
    
    if len(args) != 1:
        parser.error("In this usage, pyxform takes one argument")
    try:
        # takes the first argument as the file to be parsed
    ...
    if isinstance(pyx_package, list):
        # not sure if this is still the right syntax for what we're trying to do, but
        # if a json file has a list of questions, instead of a survey definition, it might
        # be nice to wrap it.
        pyx_package = {'type':'survey', 'children': pyx_package}
    
    if options.question_types:
        # imports question types, if the argument is passed
    ...
    # TODO: get run the output through validate
...
def process_file_to_python_dict(fn, action=None):
    """
    files can be:
        xls     =>    get converted via ExcelSurveyReader (ie. SurveyReader)
        jsongz  =>    get unzipped and handled as json
        json    =>    get handled as a python dict
    """
